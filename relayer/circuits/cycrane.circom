...
template CycraneVerifier(max_header_bytes, max_body_bytes, n, k, pack_size, expose_from) {
    ...
    // Body reveal vars
    // var max_transaction_len = 1536;
    // var max_transaction_packed_bytes = count_packed(max_transaction_len, pack_size); // ceil(max_num_bytes / 7)
    ...
    // reveal_twitter_packed <== ShiftAndPack(max_body_bytes, max_transaction_len, pack_size)(in_body_padded, transaction_calldata_idx);
}
...
component main { public [ in_body_padded ] } = TwitterVerifier(1024, 1536, 121, 17, 7, 0);